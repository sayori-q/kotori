...
def is_ffmpeg_recording():
    ret = -2
    if KotoriData.ffmpeg_proc:
        if KotoriData.ffmpeg_proc.processId() == 0:
            KotoriData.ffmpeg_proc = None
            ret = True
        else:
            ret = False
    return ret
...
def exit_handler(exit_code, exit_status):
    if exit_status != QtCore.QProcess.ExitStatus.NormalExit:
        ...
        if KotoriData.show_record_exception:
            try:
                if (
                    KotoriData.ffmpeg_proc
                    and KotoriData.ffmpeg_proc.processId() == 0
                    and KotoriData.ffmpeg_proc.exitCode() == exit_code
                    and KotoriData.ffmpeg_proc.exitStatus() == exit_status
                ):
                    standard_output = KotoriData.ffmpeg_proc.readAllStandardOutput()
                    try:
                        standard_output = bytes(standard_output).decode("utf-8")
                        standard_output = "\n".join(standard_output.split("\n")[-15:])
                    except Exception:
                        pass
                    standard_error = KotoriData.ffmpeg_proc.readAllStandardError()
                    try:
                    ...
                    KotoriData.show_record_exception(
                        _("ffmpeg crashed!") + "\n"
                    ...
                    if KotoriData.ffmpeg_processes:
                        for ffmpeg_process in KotoriData.ffmpeg_processes:
                            if (
                                ffmpeg_process
            ...
            if not ffmpeg_process_found:
                KotoriData.show_record_exception(_("ffmpeg crashed!"))
...
def record(
    ...
    if not is_return:
        KotoriData.ffmpeg_proc = QtCore.QProcess()
        KotoriData.ffmpeg_proc.start("ffmpeg", arr)
        KotoriData.ffmpeg_proc.finished.connect(exit_handler)
    else:
...
def stop_record():
    if KotoriData.ffmpeg_proc:
        KotoriData.ffmpeg_proc.terminate()


def init_record(show_exception, ffmpeg_processes):
    KotoriData.show_record_exception = show_exception
    KotoriData.ffmpeg_processes = ffmpeg_processes
