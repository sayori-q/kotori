...
class KotoriData:
    local_dir = Path(os.environ["HOME"], ".config", "kotori")
    write_lock = False
...
def read_option(name):
    ...
    if os.path.isfile(Path(KotoriData.local_dir, "player_data.json")):
        options_file = open(
            str(Path(KotoriData.local_dir, "player_data.json")), "r", encoding="utf8"
        )
...
def write_option(name, value):
    ...
    options_file = open(
        str(Path(KotoriData.local_dir, "player_data.json")), "w", encoding="utf8"
    )
