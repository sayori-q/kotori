...
from kotori.epg import worker, is_program_actual, load_epg_cache, save_epg_cache
from kotori.record import (
    record,
    record_return,
    stop_record,
    is_ffmpeg_recording,
...
from kotori.catchup import (
    get_catchup_url,
    parse_specifiers_now_url,
    format_url_clean,
    format_catchup_array,
)
from kotori.channel_logos import channel_logos_worker
from kotori.settings import parse_settings
from kotori.qt6compat import _exec
...
logger = logging.getLogger("kotori")
...
setproctitle.setproctitle("kotori")
try:
    setproctitle.setthreadtitle("kotori")
except Exception:
    pass
...
def show_exception(e, e_traceback="", prev=""):
    if e_traceback:
        e = e_traceback.strip()
    message = "{}{}\n\n{}".format(_("kotori error"), prev, str(e))
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("")
        logger.info(f"kotori version: {APP_VERSION}")
        ...
        if is_dark_theme:
            ...
            KotoriData.use_dark_icon_theme = True
        else:
            KotoriData.use_dark_icon_theme = False
        ...
        def getArrayItem(arr_item):
            ...
            elif arr_item in KotoriData.movies:
                arr_item_ret = KotoriData.movies[arr_item]
            else:
                try:
                    if " ::: " in arr_item:
                        arr_item_split = arr_item.split(" ::: ")
                        for season_name in KotoriData.series[
                            arr_item_split[2]
                        ].seasons.keys():
                            season = KotoriData.series[arr_item_split[2]].seasons[
                                season_name
                            ]
        ...
        if not os.path.isfile(str(Path(LOCAL_DIR, "playlistcache.json"))):
                        ...
                        try:
                            ...
                            for movie1 in xt.series:
                                if isinstance(movie1, Serie):
                                    KotoriData.series[movie1.name] = movie1
                    ...
                    for m3u_datai in m3u_data_got:
                        if "tvg-group" in m3u_datai:
                            if m3u_datai["tvg-group"].lower() == "vod" or m3u_datai[
                                ...
                                KotoriData.movies[m3u_datai["title"]] = m3u_datai
                            else:
                                KotoriData.series, is_matched = parse_series(
                                    m3u_datai, KotoriData.series
                                )
                                if not is_matched:
            ...
            logger.info(
                "{} channels, {} groups, {} movies, {} series".format(
                    ...
                    len(KotoriData.movies),
                    len(KotoriData.series),
                )
            )
        ...
        TV_ICON = QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "tv.png")))
        MOVIE_ICON = QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "movie.png")))
        ...
        def keyseq_ok_clicked():
            if KotoriData.selected_shortcut_row != -1:
                ...
                if not shortcut_taken:
                    shortcuts_table.item(KotoriData.selected_shortcut_row, 1).setText(
                        sel_keyseq
                    )
        ...
        help_win.setWindowTitle(_("&About kotori").replace("&", ""))
        ...
        m3u_file_1.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "file.png"))))
        m3u_file_1.clicked.connect(m3u_file_1_clicked)
        epg_file_1 = QtWidgets.QPushButton()
        epg_file_1.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "file.png"))))
        ...
        kotori_icon = QtWidgets.QLabel()
        kotori_icon.setPixmap(TV_ICON.pixmap(QtCore.QSize(32, 32)))
        kotori_label = QtWidgets.QLabel()
        myFont6 = QtGui.QFont()
        myFont6.setPointSize(11)
        myFont6.setBold(True)
        kotori_label.setFont(myFont6)
        kotori_label.setTextFormat(QtCore.Qt.TextFormat.RichText)
        kotori_label.setText(
            '<br>&nbsp;<span style="color: #b35900;">Kotori</span><br>'
        )

        kotori_widget = QtWidgets.QWidget()
        kotori_layout = QtWidgets.QHBoxLayout()
        kotori_layout.addWidget(kotori_icon)
        kotori_layout.addWidget(kotori_label)
        kotori_widget.setLayout(kotori_layout)

        kotori_layout.setAlignment(
            QtCore.Qt.AlignmentFlag.AlignHCenter | QtCore.Qt.AlignmentFlag.AlignTop
        )
        ...
        playlists_win_widget_main_layout.addWidget(kotori_widget)
        ...
        KotoriData.bitrate_failed = False
        ...
        def on_bitrate(prop, bitrate):
            try:
                ...
                if not KotoriData.bitrate_failed:
                    KotoriData.bitrate_failed = True
        ...
        def mpv_override_stop(ignore=False):
            ...
            player.play(str(Path("kotori", ICONS_FOLDER, "main.png")))
            player.pause = True
            if event_handler:
                try:
                    event_handler.on_title()
        ...
        def stopPlayer(ignore=False):
            try:
                ...
                player.loop = True
                mpv_override_play(str(Path("kotori", ICONS_FOLDER, "main.png")))
                player.pause = True
        ...
        sm3ufile.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "file.png"))))
        sm3ufile.clicked.connect(m3u_select)
        sm3uupd = QtWidgets.QPushButton()
        sm3uupd.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "update.png"))))
        sm3uupd.clicked.connect(update_m3u)
        sm3uupd.setToolTip(_("Update"))

        sepgfile = QtWidgets.QPushButton()
        sepgfile.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "file.png"))))
        sepgfile.clicked.connect(epg_select)
        sepgupd = QtWidgets.QPushButton()
        sepgupd.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "update.png"))))
        sepgupd.clicked.connect(force_update_epg_act)
        sepgupd.setToolTip(_("Update"))

        sfolder = QtWidgets.QPushButton()
        sfolder.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "file.png"))))
        ...
        def shortcuts_table_clicked(row1, column1):
            if column1 == 1:  # keybind
                ...
                KotoriData.selected_shortcut_row = row1
                keyseq.setFocus()
        ...
        def set_mpv_osc(osc_value):
            if osc_value != KotoriData.osc:
                KotoriData.osc = osc_value
        ...
        def init_mpv_player():
            ...
            try:
                player["audio-client-name"] = "kotori"
            except Exception:
            ...
            def pause_handler(unused=None, unused2=None, unused3=None):
                ...
                try:
                    if not player.pause:
                        label3.setIcon(
                            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "pause.png")))
                        )
                        label3.setToolTip(_("Pause"))
                    else:
                        label3.setIcon(
                            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "play.png")))
                        )
            ...
            def kotori_track_set(track, type1):
                global player_tracks
            ...
            init_menubar_player(
                ...
                redraw_menubar,
                QtGui.QIcon(
                    QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "circle.png"))).pixmap(
                        8, 8
                    )
                ),
                my_up_binding_execute,
        ...
        class MainWindow(QtWidgets.QMainWindow):
            ...
            def createMenuBar_mw(self):
                ...
                init_kotori_menubar(self, app, self.menu_bar_qt)
        ...
        loading_movie = QtGui.QMovie(str(Path("kotori", ICONS_FOLDER, "loading.gif")))
        ...
        loading_movie2 = QtGui.QMovie(
            str(Path("kotori", ICONS_FOLDER, "recordwait.gif"))
        )
        ...
        def dockWidget_out_clicked():
            ...
            if not fullscreen:
                # Entering fullscreen
                if not KotoriData.fullscreen_locked:
                    KotoriData.fullscreen_locked = True
                    ...
                    KotoriData.fullscreen_locked = False
            else:
                # Leaving fullscreen
                if not KotoriData.fullscreen_locked:
                    KotoriData.fullscreen_locked = True
        ...
        def mpv_mute():
            ...
            if player.mute:
                if old_value > 50:
                    label6.setIcon(
                        QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "volume.png")))
                    )
                else:
                    label6.setIcon(
                        QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "volume-low.png")))
                    )
                ...
                label6.setIcon(
                    QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "mute.png")))
                )
        ...
        tvguide_close_lbl.setPixmap(
            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "close.png"))).pixmap(32, 32)
        )
        tvguide_close_lbl.setStyleSheet(
            "background-color: {};".format(
                "black" if KotoriData.use_dark_icon_theme else "white"
            )
        )
                ...
                if settings["channellogos"] != 3:
                    if channel_logos_request != channel_logos_request_old:
                        ...
                        channel_logos_process = get_context("spawn").Process(
                            name="[kotori] channel_logos_worker",
        ...
        def playmode_change(self=False):
            KotoriData.playmodeIndex = playmode_selector.currentIndex()
        ...
        sort_upbtn.setIcon(
            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "arrow-up.png")))
        )
        sort_upbtn.clicked.connect(sort_upbtn_clicked)
        sort_downbtn = QtWidgets.QPushButton()
        sort_downbtn.setIcon(
            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "arrow-down.png")))
        )
        ...
        def channelfilter_do():
            try:
                ...
                filter_txt1 = ""
            if KotoriData.playmodeIndex == 0:  # TV channels
                btn_update.click()
            elif KotoriData.playmodeIndex == 1:  # Movies
            ...
            elif KotoriData.playmodeIndex == 2:  # Series
                try:
                    redraw_series()
                except Exception:
        ...
        def movies_group_change():
            ...
            if KotoriData.movies:
                current_movies_group = movies_combobox.currentText()
                if current_movies_group:
                    win.moviesWidget.clear()
                    currentMoviesGroup = {}
                    for movies1 in KotoriData.movies:
                        if "tvg-group" in KotoriData.movies[movies1]:
                            if (
                                KotoriData.movies[movies1]["tvg-group"]
                                == current_movies_group
                            ):
                                win.moviesWidget.addItem(
                                    KotoriData.movies[movies1]["title"]
                                )
                                currentMoviesGroup[
                                    KotoriData.movies[movies1]["title"]
                                ] = KotoriData.movies[movies1]
            else:
                win.moviesWidget.clear()
        ...
        for movie_combobox in KotoriData.movies:
            if "tvg-group" in KotoriData.movies[movie_combobox]:
                if KotoriData.movies[movie_combobox]["tvg-group"] not in movies_groups:
                    movies_groups.append(KotoriData.movies[movie_combobox]["tvg-group"])
        ...
        def redraw_series():
            KotoriData.serie_selected = False
            win.seriesWidget.clear()
            if KotoriData.series:
                for serie2 in KotoriData.series:
        ...
        def series_change(series_item):
            ...
            if sel_serie == "< " + _("Back"):
                ...
                if KotoriData.serie_selected:
                    ...
                    try:
                        if not KotoriData.series[sel_serie].seasons:
                            xt.get_series_info_by_id(KotoriData.series[sel_serie])
                        for season_name in KotoriData.series[sel_serie].seasons.keys():
                            season = KotoriData.series[sel_serie].seasons[season_name]
                        ...
                        KotoriData.serie_selected = True
        ...
        page_box.setStyleSheet(
            ...
            QSpinBox::down-button  {
              ...
              left: 1px;
              image: url("""
            + str(Path("kotori", ICONS_FOLDER, "leftarrow.png"))
            + """);
              height: 24px;
              width: 24px;
            }
            ...
            QSpinBox::up-button  {
              ...
              right: 1px;
              image: url("""
            + str(Path("kotori", ICONS_FOLDER, "rightarrow.png"))
            + """);
              height: 24px;
              width: 24px;
            }
        ...
        def showhideeverything():
            ...
            if not fullscreen:
                if dockWidget.isVisible():
                    KotoriData.compact_mode = True
                    dockWidget.hide()
                    dockWidget2.hide()
                    win.menu_bar_qt.hide()
                else:
                    KotoriData.compact_mode = False
                    dockWidget.show()
                    dockWidget2.show()
                    win.menu_bar_qt.show()
        ...
        try:
            ...
            class MyAppAdapter(MprisAdapter):
                def metadata(self) -> dict:
                    ...
                    metadata = {
                        "mpris:trackid": f"/com/sayori/kotori/playlist/{mpris_trackid}",
            ...
            mpris = Server("kotori_" + str(os.getpid()), adapter=my_adapter)
        ...
        record_icon = QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "record.png")))
        record_stop_icon = QtGui.QIcon(
            str(Path("kotori", ICONS_FOLDER, "stoprecord.png"))
        )

        label3 = QtWidgets.QPushButton()
        label3.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "pause.png"))))
        label3.setToolTip(_("Pause"))
        label3.clicked.connect(mpv_play)
        label4 = QtWidgets.QPushButton()
        label4.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "stop.png"))))
        label4.setToolTip(_("Stop"))
        label4.clicked.connect(mpv_stop)
        label5 = QtWidgets.QPushButton()
        label5.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "fullscreen.png"))))
        label5.setToolTip(_("Fullscreen"))
        label5.clicked.connect(mpv_fullscreen)
        label5_0 = QtWidgets.QPushButton()
        label5_0.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "folder.png"))))
        ...
        label5_2.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "calendar.png"))))
        label5_2.setToolTip(_("Recording scheduler"))
        label5_2.clicked.connect(show_scheduler)
        label6 = QtWidgets.QPushButton()
        label6.setIcon(QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "volume.png"))))
        ...
        hdd_gif_label.setPixmap(
            QtGui.QIcon(str(Path("kotori", ICONS_FOLDER, "hdd.png"))).pixmap(
                QtCore.QSize(32, 32)
            )
        )
        ...
        def myExitHandler_before():
            ...
            try:
                write_option(
                    "compactstate",
                    {
                        "compact_mode": KotoriData.compact_mode,
                        "playlist_hidden": KotoriData.playlist_hidden,
                        "controlpanel_hidden": KotoriData.controlpanel_hidden,
                    },
                )
            except Exception:
                pass
            try:
                write_option("volume", int(KotoriData.volume))
            except Exception:
                pass
            save_player_tracks()
            saveLastChannel()
        ...
        KotoriData.connprinted = False

        def check_connection():
            global x_conn
            if settings["autoreconnection"]:
                if playing_group == 0:
                    if not KotoriData.connprinted:
                        KotoriData.connprinted = True
        ...
        def key_t():
            ...
            if not fullscreen:
                if dockWidget.isVisible():
                    KotoriData.playlist_hidden = True
                    dockWidget.hide()
                else:
                    KotoriData.playlist_hidden = False
                    dockWidget.show()

        def lowpanel_ch():
            if dockWidget2.isVisible():
                KotoriData.controlpanel_hidden = True
                dockWidget2.hide()
            else:
                KotoriData.controlpanel_hidden = False
                dockWidget2.show()
        ...
        def setShortcutState(st1):
            KotoriData.shortcuts_state = st1
        ...
        if volume_option is not None:
            ...
            KotoriData.volume = vol_remembered
        firstVolRun = False
